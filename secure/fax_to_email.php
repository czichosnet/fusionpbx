...
//send the email
	if (strlen($fax_email) > 0 && file_exists($fax_file)) {
		//prepare the message
			$tmp_subject = (($fax_email_inbound_subject_tag != '') ? "[".$fax_email_inbound_subject_tag."]" : "Fax Erhalten").": ".$fax_file_name;

			$fax_messages = str_replace("result", "Zustellung",$fax_messages);
			$fax_messages = str_replace("sender", "Absender",$fax_messages);
			$fax_messages = str_replace("pages", "Seitenanzahl",$fax_messages);

			$tmp_text_html = "<br><strong>Fax Erhalten</strong><br><br>";
			$tmp_text_html .= "Dokumentenname: ".$fax_file_name."<br>";
			$tmp_text_html .= "Faxserver: ".$fax_extension."<br>";
			$tmp_text_html .= "Informationen: ".$fax_messages."<br>";
			$tmp_text_html .= "Diese E-Mail ist eine Automatische Benachrichtigung und kann in Ihrer PBX deaktiviert werden.<br>";

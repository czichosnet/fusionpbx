...
local file2 = assert(io.open("/opt/callcenterevaluation/AGENTS-"..callcenter_name .."-TIME-" ..  os.date('%d.%m.%Y',evaluationTime) .."-new.csv","w+"))

file2:write("Jahr,Monat,Tag,Wochentag,Agent,Angenommene Anrufe, Angemeldete Zeit\n")
...
for day,agentList in pairs(agentStatsWeek) do 
   ...
   for agent,agentData in pairs(agentList) do 
      ...
      file2:write(tostring(tonumber(agentList["date"].wday)-1),",")
      file2:write(agent,",")
